...
public class SetCommonDataInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (request.getAttribute(Constant.CURRENT_URL) == null) {
            request.setAttribute(Constant.CURRENT_URL, extractCurrentURL(request, true));
        }
        if (request.getAttribute(Constant.CURRENT_URL_NO_QUERYSTRING) == null) {
            request.setAttribute(Constant.CURRENT_URL_NO_QUERYSTRING, extractCurrentURL(request, false));
        }
        if (request.getAttribute(Constant.BACK_URL) == null) {
            request.setAttribute(Constant.BACK_URL, extractBackURL(request));
        }
    ...
    private String extractBackURL(HttpServletRequest request) {
        String url = request.getParameter(Constant.BACK_URL);
