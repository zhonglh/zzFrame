 ...
 * 下载MongoDB附件
 * 
 * @param fileId	MongoDB附件UUID
 */
function downloadMongoDBFile(fileId)
{
	if (window.plus)
	{
		// App端下载
	...
	{
		// PC端下载
		location.href = $AppContext + '/third/mongodb-download/' + fileId;
	}
}
...
function getUserAvatarUrl(avatar, domainId)
{
	if ('file' == $AttachmentEngine)
		return $AppContext + '/disk-file/user-avatar/' + ('default.png' == avatar ? '' : domainId + '/') + avatar;
	else
		if ('mongodb' == $AttachmentEngine)
		{
			if ('default.png' == avatar)
				return $AppContext + '/disk-file/user-avatar/default.png';
			else
