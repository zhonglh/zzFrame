...
<%@include file="/WEB-INF/views/common/import-homejs.jspf"%>
...
<script type="text/javascript">
    ...
    function cropImgUpload(callback)
    {
        ...
        $.ajax(
            {
                url: ctx + '/third/corpper/avatar', type: 'post', data: {"imageData": result.toDataURL().toString()}, dataType: 'json', async: false,
...
// 	{
// 		$('#userHeadImg').prop('src', '/tzcp/third/avatar/temp/' + newFileName);
...
// 	});
    ...
    function logoutSystem()
    {
        ...
        {
            window.location = '/tzcp/platform/logout/3';
        });
    }
    ...
    function saveShortcut()
    {
        ...
        $.post
        (
            '/tzcp/platform/shortcut', {menuIds: lies.join(',')},
    ...
    {
        $.post('/tzcp/platform/msg/unread', function(rsp, textStatus, jqXHR)
        {
            if (200 == rsp.code)
            {
                if (0 == rsp.data.total)
                ...
                else
                {
                    ...
                    $.each(rsp.data.rows, function(index, r)
                    {
                        html +=
                            '<li>' +
                            '	<a href="javascript:openMenu(\'/tzcp/platform/msg?id=' + r.id + '\')">' +
