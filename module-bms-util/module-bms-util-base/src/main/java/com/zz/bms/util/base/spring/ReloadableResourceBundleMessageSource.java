    ...
    protected PropertiesHolder getMergedProperties(Locale locale){
        synchronized (this.cachedMergedProperties) {
            ...
            if (mergedHolder.getProperties () == null || mergedHolder.getProperties ().size () == 0) {
                if (!locale.equals (Locale.getDefault ())) for (int i = this.basenames.length - 1; i >= 0 ; i-- ) {
            ...
            // ReloadableResourceBundleMessageSource.PropertiesHolder _mergedHolder = this.cachedMergedProperties.get (Locale.getDefault ());
            // if (null == _mergedHolder) {
            // Properties _mergedProps = new Properties ();
            // _mergedHolder = new ReloadableResourceBundleMessageSource.PropertiesHolder (_mergedProps,-1);
            // for ( int i = this.basenames.length - 1 ; i >= 0 ; i-- ) {
            // List filenames = calculateAllFilenames (this.basenames[i], Locale.getDefault ());
            // for ( int j = filenames.size () - 1 ; j >= 0 ; j-- ) {
            // String filename = (String) filenames.get (j);
            // ReloadableResourceBundleMessageSource.PropertiesHolder propHolder = getProperties (filename);
            // if (propHolder.getProperties () != null) {
            // _mergedProps.putAll (propHolder.getProperties ());
            // }
            // }
            // }
            // this.cachedMergedProperties.put (Locale.getDefault (), _mergedHolder);
            // }
            //
            // }
            return mergedHolder;
