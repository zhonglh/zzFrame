...
public class StringKit {
    ...
    public static String[] getStringArray(HttpServletRequest oRequest, String strParamName, String strDefaultValue){
        if (oRequest == null || StringKit.isBlank (strParamName)) { return null; }
    ...
    /**
     * 返回 HTTP 请求的 Referer, 如果没有, 就返回默认页面值. 仅用于移动博客开发页面命名风格: // Added at 2004-10-12 // 如果前一页面的地址包含 _action.jsp , 为了避免链接出错, 就返回默认页面 2006-08-02 增加从 url 参数 referer 的判断
     *
     * @param request
    ...
    /**
     * Write the HTML base tag to support servlet forward calling relative path changed problems. Base is used to ensure that your document's relative links are associated with the proper document
     * path. The href specifies the document's reference URL for associating relative URLs with the proper document path. This element may only be used within the HEAD tag. Example: <BASE
    ...
    public static String request(HttpServletRequest request, String fieldName){
        // POST 方法的参数有编码错误
        if (request.getMethod ().equalsIgnoreCase ("POST")) { return toChi (request.getParameter (fieldName)); }
    ...
    public String[] getParameterValues(HttpServletRequest request, String name){
        ...
        if ("GBK".equalsIgnoreCase (encoding) || "GB2312".equalsIgnoreCase (encoding)) { return request.getParameterValues (name); }
    ...
    public static String paging(int page, int recordCount, int pageSize, int pageCountSize, String linkpageurl){
        ...
        // 贸澜栏肺
        if (StartPage - 1 > 0) {
            LinkText += "<a href='" + LinkPageName + "&page=1' title='到此页'>" + txtStart + "</a>";
        } else {
            LinkText += txtStart;
        }
        // 捞傈 10俺..
        if (StartPage - 1 > 0) {
            LinkText += "<a href='" + LinkPageName + "&page=" + (StartPage - 1) + "' title='到第" + pageCountSize + "页'>" + txtPrev + "</a>";
        } else {
            LinkText += txtPrev;
        }
        ...
        if (StartPage + pageCountSize - PageCount - 1 < 0) {
            LinkText += "<a href='" + LinkPageName + "&page=" + (StartPage + pageCountSize) + "' title='到第" + pageCountSize + "页'>" + txtNext + "</a>";
        } else {
            LinkText += txtNext;
        }
        // 最后一页
